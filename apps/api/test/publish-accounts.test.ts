...
import {
  GMB_NO_VIDEO, YOUTUBE_VIDEO_ONLY, buildPublishData, publishPost, publishPostToPlatform, resolveAccessToken, unsupportedMediaError,
...
describe('per-account results', () => {
  ...
  it('keeps an earlier account success when a later attempt is blocked', () => {
    const succeeded = mergePlatformResult(undefined, [ok('a', 'Page A', 'post-a')], ['a'], AT);
    const blocked = mergePlatformResult(succeeded, [], [], '2026-09-24T11:00:00.000Z', 'The selected Facebook account is no longer connected.');
    assert.deepEqual([blocked.post_id, blocked.error], ['post-a', undefined]);
    assert.equal(storedOutcome(blocked, 'a')?.post_id, 'post-a');
  });
});
...
describe('publishPost with several accounts', () => {
  ...
  it('retries only the Page that failed', async (t) => {
    ...
    t.mock.restoreAll();
    const sent = mockPhotos(t);
    const retried = await publishPost(post, ['facebook']);
    assert.deepEqual(sent.map((s) => s.pageId), [g.second.platform_account_id]);
    assert.equal(retried.status, 'published');
    const stored = (await prisma.post.findUnique({ where: { id: post.id } }))?.publish_results as
      Record<string, { post_id: string; accounts: Record<string, { post_id?: string }> }>;
    assert.equal(stored['facebook']?.post_id, `photo-${g.first.platform_account_id}`);
    assert.equal(stored['facebook']?.accounts[g.first.id]?.post_id, `photo-${g.first.platform_account_id}`);
    assert.equal(stored['facebook']?.accounts[g.second.id]?.post_id, `photo-${g.second.platform_account_id}`);
  });
  ...
  it('keeps a first Page\'s post live when a later attempt is blocked', async (t) => {
    const g = await dealerWithPages();
    mockPhotos(t);
    const post = await newPost(g.dealerId, g.author.id, { connection_ids: [g.first.id] });

    const first = await publishPost(post, ['facebook']);
    assert.equal(first.status, 'published');

    // The named Page is disconnected before the next attempt, so this platform is now blocked
    // (selectedAccountGoneMessage) rather than sent to again.
    await prisma.platformConnection.update({ where: { id: g.first.id }, data: { is_connected: false } });
    const row = await prisma.post.findUnique({ where: { id: post.id } });
    const again = await publishPost(row!, ['facebook']);

    assert.equal(again.status, 'published');
    assert.equal(again.results[0]?.post_id, `photo-${g.first.platform_account_id}`);
    const stored = (await prisma.post.findUnique({ where: { id: post.id } }))?.publish_results as
      Record<string, { post_id: string; accounts?: Record<string, unknown> }>;
    assert.equal(stored['facebook']?.post_id, `photo-${g.first.platform_account_id}`);
    assert.ok(stored['facebook']?.accounts?.[g.first.id]);
  });
...
describe('resolveAccessToken', () => {
  it('refreshes an expiring YouTube token, same as Business Profile', async (t) => {
    ...
    process.env['GOOGLE_CLIENT_SECRET'] = 'yt-secret';
    t.after(() => {
    ...
    const conn = await prisma.platformConnection.create({
      data: { dealer_id: g.dealerId, platform: 'youtube', platform_account_id: 'UC-y', access_token: 'stale', refresh_token: '1//r', token_expires_at: new Date(0), is_connected: true },
    });
    const fetchMock = t.mock.method(globalThis, 'fetch', async () => new Response(JSON.stringify({ access_token: 'fresh-youtube-token', expires_in: 3600 }), { status: 200 }));

    const token = await resolveAccessToken(conn);

    assert.equal(token, 'fresh-youtube-token');
...
describe('publishPostToPlatform (queue worker)', () => {
  ...
  it('returns a legacy result without sending or touching the account map', async (t) => {
    const g = await dealerWithPages();
    const post = await newPost(g.dealerId, g.author.id, { connection_ids: [g.first.id] });
    await prisma.post.update({
      where: { id: post.id },
      data: { publish_results: { facebook: { post_id: 'fb-legacy', url: 'https://fb.test/legacy', published_at: AT } } },
    });
    const axiosMock = t.mock.method(axios, 'post', async () => { throw new Error('should not be called'); });

    const result = await publishPostToPlatform(buildPublishData(post, 'facebook', g.first));

    assert.deepEqual(result, { platform_post_id: 'fb-legacy', url: 'https://fb.test/legacy' });
    assert.equal(axiosMock.mock.callCount(), 0);
  });

  it('stays published when a second account job fails after the first succeeded', async (t) => {
    const g = await dealerWithPages();
    const post = await newPost(g.dealerId, g.author.id, { connection_ids: [g.first.id, g.second.id] });
    mockPhotos(t);
    await publishPostToPlatform(buildPublishData(post, 'facebook', g.first));

    t.mock.restoreAll();
    mockPhotos(t, [g.second.platform_account_id]);
    await assert.rejects(() => publishPostToPlatform(buildPublishData(post, 'facebook', g.second)));

    const stored = await prisma.post.findUnique({ where: { id: post.id } });
    assert.equal(stored?.status, 'published');
    const fb = (stored?.publish_results as Record<string, { accounts: Record<string, { post_id?: string; error?: string }> }>)['facebook'];
    assert.equal(fb?.accounts[g.first.id]?.post_id, `photo-${g.first.platform_account_id}`);
    assert.equal(fb?.accounts[g.second.id]?.error, '(#200) Permissions error');
  });

  it('keeps both accounts\' results when their jobs run at the same time', async (t) => {
    const g = await dealerWithPages();
    const post = await newPost(g.dealerId, g.author.id, { connection_ids: [g.first.id, g.second.id] });
    mockPhotos(t, [g.second.platform_account_id]);
    const jobA = buildPublishData(post, 'facebook', g.first);
    const jobB = buildPublishData(post, 'facebook', g.second);

    await Promise.allSettled([publishPostToPlatform(jobA), publishPostToPlatform(jobB)]);

    const stored = await prisma.post.findUnique({ where: { id: post.id } });
    assert.equal(stored?.status, 'published');
    const fb = (stored?.publish_results as Record<string, { post_id: string; accounts: Record<string, { post_id?: string; error?: string }> }>)['facebook'];
    assert.deepEqual(Object.keys(fb?.accounts ?? {}).sort(), [g.first.id, g.second.id].sort());
    assert.equal(fb?.post_id, `photo-${g.first.platform_account_id}`);
    assert.equal(fb?.accounts[g.first.id]?.post_id, `photo-${g.first.platform_account_id}`);
    assert.equal(fb?.accounts[g.second.id]?.error, '(#200) Permissions error');
  });

  it('fails permanently, so BullMQ does not retry, when the selected account is gone or the media is unsupported', async (t) => {
    const g = await dealerWithPages();
    await prisma.platformConnection.update({ where: { id: g.first.id }, data: { is_connected: false } });
    const gonePost = await newPost(g.dealerId, g.author.id, { connection_ids: [g.first.id] });

    await assert.rejects(
      publishPostToPlatform(buildPublishData(gonePost, 'facebook', g.first)),
      (err: unknown) => err instanceof UnrecoverableError,
    );

    const ytConn = await prisma.platformConnection.create({
      data: { dealer_id: g.dealerId, platform: 'youtube', platform_account_id: 'UC-z', access_token: 'ya29.z', is_connected: true },
    });
    const imagePost = await newPost(g.dealerId, g.author.id, { platforms: ['youtube'] });

    await assert.rejects(
      publishPostToPlatform(buildPublishData(imagePost, 'youtube', ytConn)),
      (err: unknown) => err instanceof UnrecoverableError,
    );

    // Both failures are still recorded, same as any other queue-path failure.
    const gone = await prisma.post.findUnique({ where: { id: gonePost.id } });
    assert.equal(gone?.status, 'failed');
    const image = await prisma.post.findUnique({ where: { id: imagePost.id } });
